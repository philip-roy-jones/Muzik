...
export function useAuthRedirect(accessTokenRef: React.MutableRefObject<string | null>, pathname: string, router: ReturnType<typeof useRouter>) {
  useEffect(() => {
    if (!accessTokenRef.current) { // No access token, user is not logged in
  ...
  }, [accessTokenRef, pathname, router]);
}